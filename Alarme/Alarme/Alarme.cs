...
namespace Alarme
{
    public class Alarme
    {
        ...
        // control o tempo do alarme
        private int tempo;

        public int Tempo //propfull
        {
            get { return tempo; }
            set { tempo = 1000 * 60 * value; }// convertendo o valor para minutos 
        }
        ...
        {
            Thread.Sleep(this.Tempo);//esperando x minutos e executar abaixo o beep
            if (this.EfeitoSonoro == 1)
            {
                this.Mario();
            }
            else
            {
                if (this.EfeitoSonoro == 2)
                { this.GetHashCode(); }
                else
                {
                    Console.Beep(this.EfeitoSonoro, 1000);//frequencia 250 e duracao de 1 minuto}
